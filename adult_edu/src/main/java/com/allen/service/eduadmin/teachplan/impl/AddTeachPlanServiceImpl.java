...
public class AddTeachPlanServiceImpl implements AddTeachPlanService {
    ...
    public JSONObject add(HttpServletRequest request, TeachPlan teachPlan) throws Exception {
        TeachPlan teachPlan2 = teachPlanDao.findBySchoolIdAndTypeIdAndLevelIdAndSpecIdAndYearAndTerm(teachPlan.getSchoolId(), teachPlan.getTypeId(), teachPlan.getLevelId(), teachPlan.getSpecId(), teachPlan.getYear(), teachPlan.getTerm());
        if (null != teachPlan2 && 0 < teachPlan2.getId()){
            throw new BusinessException("您设置的教学计划已经存在，如需更改请先删除后再重新上传");
        }
        teachPlan.setCenterId(UserUtil.getLoginUserForCenterId(request));
        teachPlan.setCerator(UserUtil.getLoginUserForName(request));
        teachPlan.setOperator(UserUtil.getLoginUserForName(request));
