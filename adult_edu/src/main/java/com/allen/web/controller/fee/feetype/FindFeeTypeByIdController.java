 ...
 */
@Controller
@RequestMapping(value = "/findFeeTypeById")
public class FindFeeTypeByIdController extends BaseController {
    ...
    public String open(@RequestParam("id")long id,
        ...
        request.setAttribute("school", findSchoolByIdService.find(feeType.getSchoolId()));
        request.setAttribute("type", findRecruitTypeByIdService.find(feeType.getTypeId()));
        request.setAttribute("level", findLevelByIdService.find(feeType.getLevelId()));
        request.setAttribute("reqParams", new String(reqParams.getBytes("iso-8859-1"), "gbk"));
        return "fee/feetype/info";
    }
}
